...
class DBConn():
	...
	def __init__(self, config_file):
		#self.config_file = yaml.load(open('config.yml', 'r', encoding='utf-8'))
		self.path_to_key = config_file['path_to_key']
		self.pkey = paramiko.RSAKey.from_private_key_file(self.path_to_key)
		self.sql_hostname = config_file['sql_hostname']
		self.sql_username = config_file['sql_username']
		self.sql_password = config_file['sql_password']
		self.sql_database = config_file['sql_database']
		self.sql_port = config_file['sql_port']
		self.ssh_host = config_file['ssh_host']
		self.ssh_user = config_file['ssh_user']
		self.ssh_port = config_file['ssh_port']
		self.conn, self.tunnel = self.start_conn()
